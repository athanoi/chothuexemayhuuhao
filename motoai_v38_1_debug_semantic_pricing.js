...
function naturalize(t){ if(!t) return t; let s=” “+t+” “; s=s.replace(/\s+ạ([.!?,\s]|$)/gi,”$1”).replace(/\s+nhé([.!?,\s]|$)/gi,”$1”).replace(/\s+nha([.!?,\s]|$)/gi,”$1”); s=s.replace(/\s{2,}/g,” “).trim(); if(!/[.!?]$/.test(s)) s+=”.”; return s.replace(/../g,”.”); }
function looksVN(s){ if(/[ăâêôơưđà-ỹ]/i.test(s)) return true; const hits=(s.match(/\b(xe|thuê|giá|liên hệ|hà nội|cọc|giấy tờ)\b/gi)||[]).length; return hits>=2; }
function stripMdAndLinks(s){ if(!s) return s; let x=String(s); x=x.replace(/https?://\S+/gi,’’); x=x.replace(/![^]][^)]+/g,’’); x=x.replace(/([^]+)][^)]+/g,’$1’); x=x.replace(/[*_~`]{1,3}/g,’’); x=x.replace(/\s{2,}/g,’ ’).trim(); return x; }

const K = { sess:“MotoAI_v38_session”, ctx:“MotoAI_v38_ctx”, learn:“MotoAI_v38_learn”, autoprices:“MotoAI_v38_auto_prices”, stamp:“MotoAI_v38_learnStamp”, clean:“MotoAI_v38_lastClean”, dbg:“MotoAI_v38_debug_stats” };

const CSS =  :root{--mta-z:2147483647;--m-blue:${CFG.themeColor};--m-bg:#fff;--m-text:#0b1220;--m-in-h:34px;--m-in-fs:15px;--m-send-size:36px} #mta-root{position:fixed;right:16px;bottom:calc(16px + env(safe-area-inset-bottom,0));z-index:var(--mta-z);font-family:-apple-system,system-ui,Segoe UI,Roboto,"Helvetica Neue",Arial,sans-serif} #mta-bubble{width:56px;height:56px;border:none;border-radius:999px;background:linear-gradient(150deg,var(--m-blue),#00B2FF);display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 10px 28px rgba(0,0,0,.18);color:#fff;font-size:22px} #mta-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.2);opacity:0;pointer-events:none;transition:opacity .15s ease} #mta-backdrop.show{opacity:1;pointer-events:auto} #mta-card{position:fixed;right:16px;bottom:16px;width:min(420px,calc(100% - 24px));height:70vh;max-height:740px;background:var(--m-bg);color:var(--m-text);border-radius:18px;box-shadow:0 12px 40px rgba(0,0,0,.25);display:flex;flex-direction:column;overflow:hidden;transform:translateY(110%);transition:transform .22s cubic-bezier(.22,1,.36,1)} #mta-card.open{transform:translateY(0)} #mta-header{background:linear-gradient(130deg,var(--m-blue),#00B2FF);color:#fff} #mta-header .bar{display:flex;align-items:center;gap:10px;padding:11px 12px} #mta-header .avatar{width:30px;height:30px;border-radius:50%;background:rgba(255,255,255,.25);display:flex;align-items:center;justify-content:center;font-size:15px} #mta-header .info{display:flex;flex-direction:column;gap:2px} #mta-header .name{font-weight:700;font-size:14px;line-height:1.1} #mta-header .status{font-size:12px;opacity:.9;display:flex;align-items:center;gap:4px} #mta-header .status-dot{width:8px;height:8px;border-radius:50%;background:#3fff6c} #mta-header .actions{margin-left:auto;display:flex;gap:6px;align-items:center} #mta-header .act{width:28px;height:28px;border-radius:999px;background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.25);display:flex;align-items:center;justify-content:center;color:#fff;font-size:13px;text-decoration:none} #mta-close{background:none;border:none;color:#fff;font-size:20px;cursor:pointer} #mta-body{flex:1;overflow-y:auto;background:linear-gradient(180deg,#E9EEF5 0%, #D7E0EC 100%);padding:14px 10px 12px;scroll-behavior:smooth} .m-msg{max-width:78%;margin:6px 0;padding:8px 11px;border-radius:20px;line-height:1.45;word-break:break-word;box-shadow:0 1px 1px rgba(0,0,0,.05);font-size:14px} .m-msg.bot{background:#fff;color:#0d1117;border:1px solid rgba(0,0,0,.03)} .m-msg.user{background:#0084FF;color:#fff;margin-left:auto;border-bottom-right-radius:3px} #mta-typing{display:inline-flex;gap:6px;align-items:center;margin:6px 0} #mta-typing span{background:#fff;padding:6px 8px;border-radius:999px;font-size:12px} #mta-tags{background:#f6f7f9;border-top:1px solid rgba(0,0,0,.05);transition:max-height .2s ease,opacity .2s ease} #mta-tags.hidden{max-height:0;opacity:0;overflow:hidden} #mta-tags .track{display:block;white-space:nowrap;overflow-x:auto;padding:8px 10px 10px} #mta-tags button{display:inline-block;margin-right:8px;background:#fff;border:1px solid rgba(0,0,0,.05);border-radius:999px;padding:6px 12px;font-size:13px;cursor:pointer} #mta-input{background:#fff;border-top:1px solid rgba(0,0,0,.05);padding:8px 8px;display:flex;gap:8px;align-items:center;position:sticky;bottom:0} #mta-in{flex:1;border:1px solid rgba(0,0,0,.12);height:var(--m-in-h);line-height:var(--m-in-h);padding:0 14px;border-radius:calc(var(--m-in-h)/2);background:#F2F4F7;color:#0b1220;font-size:var(--m-in-fs);box-sizing:border-box;-webkit-appearance:none;appearance:none} #mta-in::placeholder{color:rgba(0,0,0,.45)} #mta-send{width:var(--m-send-size);height:var(--m-send-size);border:none;border-radius:50%;background:linear-gradient(160deg,#0084FF,#00B2FF);color:#fff;cursor:pointer;box-shadow:0 6px 18px rgba(0,132,255,.4);font-size:15px;display:flex;align-items:center;justify-content:center} @media(max-width:520px){#mta-card{right:8px;left:8px;width:auto;height:70vh}#mta-body{padding-bottom:8px}} @media(prefers-color-scheme:dark){ :root{--m-bg:#1a1c20;--m-text:#ecf0f5} #mta-body{background:linear-gradient(180deg,#131416,#1a1c20)} .m-msg.bot{background:#23252a;color:#fff;border:1px solid rgba(255,255,255,.03)} #mta-input{background:#1a1c20;border-top:1px solid rgba(255,255,255,.04)} #mta-in{background:#121317;color:#fff;border:1px solid rgba(255,255,255,.12)} #mta-in::placeholder{color:rgba(255,255,255,.66)} #mta-tags{background:#1c1e22;border-top:1px solid rgba(255,255,255,.05)} #mta-tags button{background:#22242a;color:#fff;border:1px solid rgba(255,255,255,.1)} };
const HTML = `

  <div id="mta-root" aria-live="polite">
    <button id="mta-bubble" aria-label="Mở chat cùng ${CFG.brand}" aria-expanded="false">💬</button>
...
const safeText = role===“bot” ? stripMdAndLinks(text) : text;
...
const TYPE_ALIASES = [
{canon:‘vision’,     re:/\bvi\ssion\b|\bvision\b|\bvis\b/i},
{canon:‘air blade’,  re:/\bair\sblade\b|\bairblade\b|\bab\b/i},
{canon:‘50cc’,       re:/\b50\scc\b|\b50cc\b/i},
{canon:‘xe điện’,    re:/\bxe\sđiện\b|vinfast|yadea|dibao|gogo|klara|evo|impes/i},
{canon:‘xe côn tay’, re:/\b(côn\stay|tay\scôn|exciter|ex\b|winner|win\b|raider|w175|msx|cb150|cbf190)\b/i},
{canon:‘xe ga’,      re:/\b(sh\smode|sh|lead|vespa|liberty|grande|janus|pcx|attila|mio|novo)\b|\bxe\sga\b/i},
{canon:‘xe số’,      re:/\b(wave(\salpha)?|alpha|blade|sirius|jupiter|future|dream)\b|\bxe\ssố\b/i}
];
function detectType(t){ const s=(t||’’).toLowerCase().normalize(‘NFC’); for(const it of TYPE_ALIASES){ if(it.re.test(s)) return it.canon; } return null; }
function detectQty(t){ const m=(t||””).match(/(\d+)\s*(ngày|day|tuần|tuan|week|tháng|thang|month)?/i); if(!m) return null; const n=parseInt(m[1],10); if(!n) return null; let unit=“ngày”; if(m[2]){ if(/tuần|tuan|week/i.test(m[2])) unit=“tuần”; else if(/tháng|thang|month/i.test(m[2])) unit=“tháng”; } return {n,unit}; }
function detectIntent(t){
return {
needPrice:   /(giá|gia\b|bao nhiêu|thuê|tính tiền|cost|price)/i.test(t),
...
function baseFor(type,unit){ const it=PRICE_TABLE[type]; if(!it) return null; const key=unit===“tuần”?“week”:(unit===“tháng”?“month”:“day”); const arr=it[key]; if(!arr) return null; return Array.isArray(arr)?arr[0]:arr; }
function extractPricesFromText(txt){
const clean=String(txt||’’);
const lines=clean.replace(/<script[\s\S]?</script>/gi,’ ’).replace(/<style[\s\S]?</style>/gi,’ ‘).replace(/<[^>]+>/g,’ ‘).replace(/\s+/g,’ ‘).split(/[\n.•-–]|<br\s*/?>/i);
const out=[]; const reNum=/(\d{2,3}(?:[.,]\d{3})+|\d{4,})(?:\s*(?:vnđ|vnd|đ|d|k))?/i;
const models=[
{key:/\bvision\b/i,type:‘vision’},
{key:/air\sblade|airblade|\bab\b/i,type:‘air blade’},
{key:/\b50\scc\b|\b50cc\b/i,type:‘50cc’},
{key:/côn\stay|tay\scôn/i,type:‘xe côn tay’},
{key:/xe\sđiện|vinfast|yadea|dibao|gogo|klara/i,type:‘xe điện’},
{key:/wave|sirius|blade|jupiter|xe\ssố/i,type:‘xe số’},
{key:/xe\s*ga|vision|lead|vespa|liberty|grande|janus/i,type:‘xe ga’}
];
for(const raw of lines){
const line=String(raw||’’); const found=models.find(m=> m.key.test(line)); if(!found) continue;
const m=line.match(reNum); if(!m) continue;
let val=m[1].replace(/[^\d]/g,’’); if(/\bk\b/i.test(line) && parseInt(val,10)<100000) val=String(parseInt(val,10)*1000);
const price=parseInt(val,10); if(!price || price>=5000000) continue;
const unit=/\b(tháng|thang|month)\b/i.test(line)?‘tháng’:(/\b(tuần|tuan|week)\b/i.test(line)?‘tuần’:‘ngày’);
...
try{ return str.replace(/[^\p{L}\p{N}\s]+/gu,’ ‘).split(/\s+/).filter(Boolean); }
catch{ return str.replace(/[^0-9a-zA-Z\u00C0-\u1EF9\s]+/g,’ ‘).split(/\s+/).filter(Boolean); }
}
...
function getIndexFlat(){ const cache=loadLearn(); const out=[]; Object.keys(cache).forEach(key=>{ (cache[key].pages||[]).forEach(pg=> out.push(Object.assign({source:key}, pg))); }); return out; }
function buildBM25(docs){
const k1=1.5,b=0.75; const df=new Map(), tf=new Map(); let total=0;
docs.forEach(d=>{ const toks=tk(d.text); total+=toks.length; const map=new Map(); toks.forEach(t=> map.set(t,(map.get(t)||0)+1)); tf.set(d.id,map); new Set(toks).forEach(t=> df.set(t,(df.get(t)||0)+1)); });
...
qToks.forEach(t=>{ const f=map.get(t)||0; if(!f) return; const idfv=idf.get(t)||0; s+= idfv*(f*(k1+1))/(f + k1*(1 - b + b*(docLen/avgdl))); });
return s;
}
...
console.table(rows.map(r=>({domain:r.domain, urlsSeen:r.urlsSeen, pagesKept:r.pagesKept, txtPages:r.txtPages, htmlPages:r.htmlPages, nonVNSkipped:r.nonVNSkipped, noindexSkipped:r.noindexSkipped, autoPriceHits:r.autoPriceHits, durationMs:r.durationMs})));
const totals=rows.reduce((m,r)=>({urlsSeen:m.urlsSeen+r.urlsSeen, pagesKept:m.pagesKept+r.pagesKept, txtPages:m.txtPages+r.txtPages, htmlPages:m.htmlPages+r.htmlPages, nonVNSkipped:m.nonVNSkipped+r.nonVNSkipped, noindexSkipped:m.noindexSkipped+r.noindexSkipped, autoPriceHits:m.autoPriceHits+r.autoPriceHits, durationMs:m.durationMs+r.durationMs}), {urlsSeen:0,pagesKept:0,txtPages:0,htmlPages:0,nonVNSkipped:0,noindexSkipped:0,autoPriceHits:0,durationMs:0});
...
if(intents.needContact) return polite(anh/chị gọi ${CFG.phone} hoặc nhắn Zalo qua số ${CFG.phone} là có người nhận ngay.);
...
const p50=arr[Math.floor(arr.length0.50)];
if(PRICE_TABLE[t]){
...
function showTyping(){ const body=$(”#mta-body”); if(!body) return; const box=document.createElement(“div”); box.id=“mta-typing”; box.innerHTML=<span>Đang nhập</span>; body.appendChild(box); body.scrollTop=body.scrollHeight; }
function hideTyping(){ const t=$(”#mta-typing”); if(t) t.remove(); }
function ensureInputVisible(){ const inp=$(”#mta-in”), body=$(”#mta-body”); if(!inp||!body) return; try{ inp.scrollIntoView({block:‘nearest’, inline:‘nearest’}); }catch{} body.scrollTop=body.scrollHeight; }
function setAriaOpen(open){ const card=$(”#mta-card”), bubble=$(”#mta-bubble”); if(card) card.setAttribute(“aria-hidden”, open ? “false” : “true”); if(bubble) bubble.setAttribute(“aria-expanded”, open ? “true” : “false”); }
...
const b=$(”#mta-bubble”); if(b) b.focus();
}
function autoAvoid(){
...
$(”#mta-send”).addEventListener(“click”, ()=>{ const inp=$(”#mta-in”); const v=inp.value.trim(); if(!v) return; inp.value=””; sendUser(v); });
...
document.addEventListener(“keydown”, (e)=>{ if(e.key===“Escape” && isOpen){ e.preventDefault(); closeChat(); } }, {passive:false});
}
...
if(!last || (Date.now()-last) >= CFG.refreshHours3600*1000){
